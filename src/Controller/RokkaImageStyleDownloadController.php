...
namespace Drupal\rokka\Controller;
...
class RokkaImageStyleDownloadController extends ImageStyleDownloadController {
    ...
    /**
     * Generates a Rokka derivative, given a style and image path.
     *
     ...
     */
    public function deliver(Request $request, $scheme, ImageStyleInterface $imageStyle)
    {
        dump($scheme);
        dump($imageStyle);
        die();
        ...
        // starts with styles/.
        $valid = !empty($imageStyle) && \Drupal::service('file_system')->validScheme($scheme);
        if (!$this->config('image.settings')->get('allow_insecure_derivatives') || strpos(ltrim($target, '\/'), 'styles/') === 0) {
            $valid &= $request->query->get(IMAGE_DERIVATIVE_TOKEN) === $imageStyle->getPathToken($image_uri);
        }
        if (!$valid) {
            throw new AccessDeniedHttpException();
        }

        $derivative_uri = $imageStyle->buildUri($image_uri);
        ...
        if (!file_exists($derivative_uri)) {
            $lock_name = 's3fs_image_style_deliver:' . $imageStyle->id() . ':' . Crypt::hashBase64($image_uri);
        ...
        if (!$success) {
            ...
            $success = $imageStyle->createDerivative($image_uri, $derivative_uri) &&
